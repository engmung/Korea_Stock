        ...
        async with API_SEMAPHORE:
            ...
            def call_gemini():
                try:
                    ...
                    system_instruction = f"""내가 주는 스크립트의 내용을 필요한 내용만 정리한 보고서로 만들어줘.
내가 주식종목채널의 콘텐츠를 주면, 너는 거기서 언급되는 종목들과 그 내용을 상세하게 기록해줘.
언급된 이유, 추천정도, 투자관점 등등 자세하게. 다른 내용은 필요 없고, 오로지 종목과 그에 관한 정보만 줘. 
시장 전반에 대한 언급같은 건 필요 없어.

중요 규칙:
1. 오직 구체적인 개별 종목만 포함하세요. 종목명을 명확하게 언급하지 않은 섹터, 테마, 업종 등은 절대 포함하지 마세요.
2. 단순히 과거의 매매 성공 사례를 설명하기만 하고 현재 매수/매도 의견이 없는 종목은 제외하세요.
3. 종목명은 정확하게 작성하세요.
4. "화장품주", "건설 관련주" 같은 섹터나 그룹은 포함하지 말고 구체적인 회사명만 포함하세요.
"""
