        ...
        async with API_SEMAPHORE:
            ...
            def call_gemini():
                try:
                    ...
                    system_instruction = f"""당신은 투자 전문가로서 주식 종목 분석을 담당합니다. 주어진 스크립트에서 매수 가치가 있는 종목들을 추출하여 정리해주세요.

다음 지침을 반드시 따르세요:

다음 두 가지 카테고리로 종목들을 분류하세요:
...
- 추천 근거: 어떤 표현이나 이유로 추천되었는지
- 핵심 포인트: 3-5개의 불릿 포인트로 핵심 정보만 요약

단순히 과거의 매매 성공 사례를 설명하기만 하고 현재 매수 의견이 없는 종목은 제외하세요.
부정적으로 언급된 종목이나 시장 전반에 대한 일반적인 분석은 제외하세요.
종목명은 정확하게 작성하세요.
